...
describe('direct course navigation',()=>{
  it('offers all six home destinations in navigation order',()=>{
    ...
    expect([...cards!.matchAll(/<a[^>]*href="([^"]+)"/g)].map(m=>m[1].split('/').filter(Boolean).at(-1)))
      .toEqual(['lectures','sessions','assessments','workspace','people','policies']);
    expect([...cards!.matchAll(/<h3\b[^>]*>([^<]+)<\/h3>/g)].map(m=>m[1]))
      .toEqual(['Lectures','Sessions','Assessment','Workspace','People','Policies']);
  });
  it('ships published destinations in all course layouts without changing the overview links',()=>{
    ...
    for(const route of routes){
      ...
      for(const [collection,count] of [['lectures',5],['sessions',12],['assessments',4],['workspace',3],['people',2]] as const){
