...
describe('shared lab navigation',()=>{
  it.each(['workspace','math-lab'])('ships essential separation independently of scoped CSS on %s',route=>{
    ...
    expect(nav?.match(/<a\b/g)).toHaveLength(2);expect(nav?.match(/aria-current="page"/g)).toHaveLength(1);
    expect(nav).toMatch(new RegExp(`href="[^"]*/${route}/"[^>]*aria-current="page"`));
  });
  it('uses one component on both pages and loads shared styling through site config',()=>{
    for(const route of ['workspace','math-lab'])expect(readFileSync(`src/pages/${route}/index.astro`,'utf8')).toContain(`<LabNavigation current="${route}"`);
