---
title: Defend the report
description:
  A number is not evidence. Declare the protocol behind a claim, run it
  twelve times, and find out what it will carry.
week: 11
date: 2026-10-19
arc: C
report: true
...
related:
  - lectures/week-08
  - sessions/11-why-it-persists
  - sessions/09-human-evaluation
  - assessments/final-report
---

The final report is not asked to find the true score. For the candidates on
this bench there is no instrument-free answer to find: week 8 left two honest
protocols ranking the same pair in opposite orders, and neither of them is
the mistaken one. What the report is asked for is a claim a reader can check,
which means a claim that carries the protocol that produced it.

That is a lower bar than a true number and a much higher one than a table.

## Bring the evidence, including the gaps

Bring the measurement log, a candidate reporting table, and your strongest
reason for keeping FID. Separate what you measured from what you would still
have to run. Week 9's protocol belongs in that second list too.
...
The checker can be that careful because these candidates were built here and
their differences are known. Your report has no such access, which is an
argument for more care, not less.

## Carry it into the defence

Leave with an annotated reporting table and a named limitation rather than a
claim that evaluation is solved. Prepare the case for comparability in the
[week 11 session](/sessions/11-why-it-persists/), test your proposal against
the [final report requirements](/assessments/final-report/), and defend it at
the [week 12 presentations](/sessions/12-what-you-would-report-instead/),
where the bench gets scored one last time.
